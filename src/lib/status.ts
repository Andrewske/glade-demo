...
function checkUnreviewedClientAction(contact: ContactWithContext): boolean {
  ...
  const clientActions = contact.conversations.filter(c => c.type === 'client-response')
  const mostRecentClientAction = clientActions.length > 0
  ...
  const attorneyActions = contact.conversations.filter(c =>
    c.type === 'attorney-message' || c.type === 'ai-followup'
  )
  const mostRecentAttorneyAction = attorneyActions.length > 0
