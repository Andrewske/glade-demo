...
const STATUS_CONFIG = {
	...
	"waiting-for-you": {
		...
		subtitle: "Your client is waiting on your response. Here's a quick reply:",
		icon: (
			<path
				strokeLinecap="round"
				strokeLinejoin="round"
...
export function SuggestedAction({ contact, status }: SuggestedActionProps) {
	const [escalationMessage, setEscalationMessage] = useState<string>("");
	const [loading, setLoading] = useState(true);

	const config =
		STATUS_CONFIG[status as keyof typeof STATUS_CONFIG] ||
		STATUS_CONFIG["needs-attention"];
	...
	const handleEmail = () => {
		const subject = encodeURIComponent(
			`Re: Your Chapter ${contact.chapter} Case`,
		);
		const body = encodeURIComponent(escalationMessage);
		window.open(`mailto:${contact.email}?subject=${subject}&body=${body}`);
	};

	const handleSMS = () => {
		window.open(
			`sms:${contact.phone}?body=${encodeURIComponent(escalationMessage)}`,
		);
	};
	...
	if (loading) {
		return (
			<div
				className={`rounded border ${config.borderClass} ${config.bgClass} p-6`}
			>
	...
	return (
		...
		>
			...
			<div className="flex gap-3">
				...
				<button
					onClick={handleEmail}
					className="flex items-center gap-2 rounded bg-muted px-4 py-2 text-sm font-medium text-foreground transition-colors hover:bg-muted/80"
				>
					<svg
						className="h-4 w-4"
						fill="none"
